...
Claude adaptations add guidance for connector availability, prefixed tool names, user authorization, and treating retrieved instructions as evidence. They also require discovering an existing connected GovTribe server before declaring authentication unavailable and resolving relative Documentation citations against `https://govtribe.com`. Monitoring guidance now checks the connected tool catalog instead of freezing an assertion about which automation tools the server exposes. Five relative asset links in the proposal-outline guide were corrected to resolve from its references directory. Reference guides, templates, sample artifacts, and Python helpers are otherwise preserved.
